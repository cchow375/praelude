...
codesign --force --deep --sign "$SIGN_IDENTITY" --identifier "$IDENTIFIER" "$INSTALLED_APP"
...
FOUND="$(find /Applications "$ROOT" "$HOME/Desktop" "$HOME/Downloads" -type d -name CodaKiller.app -prune -print 2>/dev/null || true)"
[[ "$FOUND" == "$INSTALLED_APP" ]] || fail "expected exactly $INSTALLED_APP; found: ${FOUND:-none}"
SPOTLIGHT="$(mdfind 'kMDItemCFBundleIdentifier == "com.christian.codakiller"' | grep '/CodaKiller.app$' || true)"
