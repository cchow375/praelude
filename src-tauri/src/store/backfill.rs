...
const PALETTE: [&str; 6] = [
    "#6b8fb5", "#b58f6b", "#8fb56b", "#b56b8f", "#6bb5a8", "#a86bb5",
];
...
fn backfill_hard_spots_for_piece(conn: &Connection, pid: i64) -> rusqlite::Result<()> {
    ...
    let raw: String = conn
        .query_row("SELECT hard_spots FROM piece WHERE id=?1", [pid], |r| {
            r.get(0)
        })
...
fn backfill_goals_for_piece(conn: &Connection, pid: i64) -> rusqlite::Result<()> {
    let existing: i64 =
        conn.query_row("SELECT count(*) FROM goal WHERE piece_id=?1", [pid], |r| {
            r.get(0)
        })?;
    if existing > 0 {
        return Ok(());
    ...
    let raw: String = conn
        .query_row("SELECT goals FROM piece WHERE id=?1", [pid], |r| r.get(0))
        .unwrap_or_else(|_| "[]".into());
...
mod tests {
    ...
    fn conn_with_v3_schema() -> Connection {
        ...
        c.execute_batch(crate::store::migrations::SCHEMA_V1)
            .unwrap();
        c.execute_batch(crate::store::migrations::SCHEMA_V2)
            .unwrap();
        c.execute_batch("PRAGMA foreign_keys = OFF;").unwrap();
        c.execute_batch(crate::store::migrations::SCHEMA_V3)
            .unwrap();
        c.execute_batch("PRAGMA foreign_keys = ON;").unwrap();
        c
    }
    ...
    fn clustering_merges_overlapping_and_touching_but_not_disjoint() {
        ...
        c.execute(
            "INSERT INTO piece (id,title,folder_path) VALUES (1,'P','/p')",
            [],
        )
        .unwrap();
        // overlapping
        ...
        let region_of = |bid: i64| -> i64 {
            c.query_row("SELECT region_id FROM rep_block WHERE id=?1", [bid], |r| {
                r.get(0)
            })
            .unwrap()
        };
