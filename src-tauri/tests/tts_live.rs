...
//!
//! Each speaks "Praelude online" audibly THROUGH the engine via the real
//! [`Speaker`] worker + half-duplex gate, proving the whole path end-to-end.

use std::sync::atomic::{AtomicUsize, Ordering};
use std::sync::Arc;

use praelude_lib::audio::{Engine, EngineConfig, EngineHandle};
use praelude_lib::tts::{select_provider, Gate, PcmSink, Speaker, SpeakerConfig, TtsProvider};
...
fn speak_through_engine(provider: Box<dyn TtsProvider>, label: &str) {
    ...
    eprintln!("[{label}] speaking 'Praelude online' ...");
    speaker
        .speak_blocking("Praelude online")
