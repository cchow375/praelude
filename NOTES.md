...
- **The next source build gives earned practice feedback a deliberate sound/animation hierarchy (2026-09-03).** Every durably committed verdict now gets a short local Web Audio acknowledgement: a bright two-note spark for Clean and quiet neutral confirmations for Sloppy/Again. Set completion becomes a compact rising fanfare; a variant-chain stage earns a three-note arc; mastery gets the fullest four-note landing. The generated palette is offline and best-effort—blocked or unavailable audio is swallowed and can never delay, fail or falsely confirm a rep write. Rep buttons also perform a short Clean spark/bloom, while set/chain overlays add deterministic geometric bursts. Reduced-motion users retain the prior single static fade. Focused frontend tests (173) plus TypeScript and production build pass; source only, not yet packaged or installed.
