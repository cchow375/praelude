...
## v11 — Studio and practical glass (2026-09-07; source candidate)
...
- **The full native result uses four test workers.** Default high-parallel execution exposed speech timing sensitivity; the sandbox also denied the speech probe. `RUST_TEST_THREADS=4` completed 1,106 passed / 18 ignored plus integrations. Record that constrained passing run, not a fictitious clean default run. Final frontend, package/install and real native UI results remain in the release QA ledger until confirmed.
- **Installed v10.1 Score file access became available during v11 work.** Computer use opened the real 25-page Scherzo score and 51 saved regions in the existing installed app; its previous Desktop permission blocker is cleared at that boundary. A fresh ad-hoc v11 launch may re-prompt and needs a new native check.
- **Cover picker automation has a precise evidence limit.** The browser image file chooser hung, so actual UI selection/upload remains unverified. Preprocessing and native save/reopen/invalid-payload tests pass; no mock or store test should be relabeled as picker acceptance. See B109.
- **The shell owns practice-panel tucking across workspaces.** Browsing Today/Pieces/Studio/Settings automatically minimizes an expanded Rep Counter into the bottom practice-tools pill. Returning to Score/Warmups restores it only if the shell tucked it; a manual closed/minimized choice stays respected, and the user can manually restore it while browsing. The earlier Studio-local unmount cleanup could reopen the Rep Counter over the library, so it is removed in favor of one shell owner. `DockProvider` records ephemeral user-action provenance and `RepPanel` identifies automatic reveal separately. Manual restore/minimize/close remain respected, including persisted and delayed-load cases. Independent final dock/library review passes with no remaining source blockers; 222 broad dock/Shell tests pass, and two additional policy cases are in the running full frontend suite.
- **v11 pre-install safety boundary is captured (2026-09-07 23:41 EDT).** Pre-install audit at 2026-09-07 23:41 EDT: schema 21, integrity OK/FK0; 11 pieces / 352 blocks / 3,237 reps / 63 sessions / 12,233 events, zero open sessions/blocks; the native app is quit. Backup `/Users/c3/Library/Application Support/Praelude Release Backups/v11.0.0-2026-09-07-2340/pre-v11.0.0.db` (35,004,416 bytes), SHA-256 `b285c5a0624769aeccc27a2543f9b414eb1b22750fa2c551e60f02cfee553cb4`; rollback `/Users/c3/Library/Application Support/Praelude Release Backups/v11.0.0-2026-09-07-2340/Praelude-10.1.0-rollback.zip` (9,778,474 bytes), SHA-256 `1b9647ce671feab84e23c0da66d0fd5351e19c1f2fc8e2908432e252a7e24d40`. Full table hashes are in repo `docs/qa/v11.0.0/pre-install-audit.json`. Install remains pending.
- **Source-freeze gates are explicit.** Independent final dock/library review passes with no remaining source blockers; 222 broad dock/Shell tests pass. TypeScript, production build, Rustfmt, native 1,106/18 ignored plus integrations and strict all-target/all-features Clippy pass. The final full frontend run, including two additional dock-policy cases, is still running; its expected count is not recorded as a result.
- **Living docs are a current entry point, not stacked installed-status history.** v11 condenses the portable summary, tutorial, Command Center and repo AGENTS. Historical release facts stay in Changelog/version records; old roadmap handoff text is explicitly archived. The installed tutorial must not call the candidate installed before package/data/launch gates pass.
