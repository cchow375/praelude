...
- **Rewards follow new saved attempt IDs, not repaired projections.** Rep and Shell completion hooks hold attempt high-water marks; polling, remounts, undo/redo and history repairs cannot replay effects. The fresh-context review caught Shell's old unsatisfied-to-satisfied correction replay and the new `useRepCompletion` hook fixes it. Final rep audio yields to the milestone cue.
- **The old audio context never initialized.** `context?.state !== "closed"` was true for null, returning null forever. Explicit non-null guarding fixes it. Gesture-wide pointer/key unlocking supports later spoken verdicts; native immediate acknowledgement remains. Felt-string plucks, glass harmonics and filtered air use bounded voices/variation, a compressor, capped setbacks, stale-resume suppression and complete disconnection. Web Audio stays best-effort after durable writes.
- **Release gates:** final frontend 2,614 passed / 1 skipped with four workers; native 1,106 passed / 18 ignored plus integrations; build/TypeScript, strict Clippy and Rustfmt pass. A sandboxed native run failed the macOS `say` probe; the authorized native run passed. One concurrently loaded frontend run timed out on the existing Listen Back capture assertion; four-worker full rerun passed without production/test changes. Independent live mock review passed 1280×720 and 720×520, actual set and two-variant chain endings, undo, audio lifecycle and reduced-motion source. Native audible quality remains a musician verdict. Exact package/install facts live in `docs/qa/v11.1.0/`.

## v11 — Studio and practical glass (installed 2026-09-07; finalized 2026-09-08)

- **Studio is a projection plus durable cosmetic transactions, not an XP counter to increment.** `store/studio/progress.rs` derives focused time from retained canonical practice events, 1 XP per 600 seconds, and validates completed-set evidence through the same effective-attempt/undo/recovery/variant/tempo projector as practice. Per-session set milestone totals are 1/2/4/6 at 3/5/7/10. A final effective attempt owns set credit; later administrative edits or close retries cannot relocate it into a new session. The bounded integer rank search remains exact through continued Encore ranks. Ranks represent commitment, never acoustic skill.
- **Listen Back tests must assert disposal after automatic re-arming.** `commitAfterVerdict` schedules the next capture with a zero-delay timer; `idle` is a transient render, so an async assertion can legitimately see `recording`. The regression test now waits for the second capture and verifies the old URL is revoked, temporary take/Keep/review state is cleared and no audio was saved. Production behavior is unchanged.
- **No schema migration or practice-history writes are needed.** `motivation.studio.v1` in `setting` stores local name, fixed catalog purchase ledger and one equipped item in each of six slots. Immediate native transactions validate revision, known items, price, rank, ownership and current derived balance before committing. Missing reads return defaults without a write; corrupt payloads refuse instead of resetting inventory. A correction may reduce earned coins below prior spending: retain possessions, clamp available balance to zero and block new spending until earnings cover it. Retried/duplicate purchases do not double-charge. Profile names normalize whitespace, require 1–40 characters and reject controls.
- **Real-copy rehearsal is exact, not count-only.** The 46-table before/after comparison changes only `motivation.studio.v1` after a profile save/purchase; integrity OK/FK0 and the full practice graph are unchanged. This copy derives 204 XP / 198 qualifying completed sets / Prelude 3 / 50 coins before purchase. The native durable reopen test proves retained profile/ownership/equipment. See `docs/qa/v11.0.0/data-rehearsal.json`; do not copy the test profile or purchases into Christian's live DB.
- **The visual scope is restrained glass in Tauri, with offline vector artwork.** Shell and controls get translucent materials; Score remains readable. Studio separates room rendering, shop, rank path, record, state and API modules. Twenty-five initial catalog items include a side table; fixed slots avoid a free-placement engine and continuous rendering cost. Owned accent palettes are distinct from the Settings Dark/Light/System mode. Accounts/friends/sync are absent; only the device-local display name is built.
- **Library artwork is bounded local data.** Chosen JPEG/PNG/WebP files are center-cropped to at most 480px and encoded as a bounded JPEG thumbnail; native validation gates stored bytes. Original images/PDFs remain untouched. Cover grid/list, normalized title/composer search, sort and Resting terminology sit atop existing safe library state and folder actions. Resting is reversible archive metadata, not deleted history.
- **Fresh-context UI review is part of the implementation boundary.** Studio mutation errors must remain visible inside the active dialog, stale-write recovery must be usable and event refreshes during writes must be queued. Shared dialogs isolate background interaction and trap/restore focus; workspace navigation resets scroll. Library menus must preserve rename-input/Save focus and close coherently on Tab/anchor invalidation. See B104–B106 and the QA ledger for final findings and rechecks.
- **The full native result uses four test workers.** Default high-parallel execution exposed speech timing sensitivity; the sandbox also denied the speech probe. `RUST_TEST_THREADS=4` completed 1,106 passed / 18 ignored plus integrations. Record that constrained passing run, not a fictitious clean default run. Final frontend 2,595/1 skipped, package/install and two fresh native launches pass; exact evidence is in the release QA ledger.
- **Installed v10.1 Score file access became available during v11 work.** Computer use opened the real 25-page Scherzo score and 51 saved regions in the existing installed app; its previous Desktop permission blocker is cleared at that boundary. The fresh ad-hoc v11 launch did re-prompt at 23:53:43: Desktop/Speech code requirements changed. Native Score/Variants remain pending Desktop Allow; native Studio/Library/Settings pass.
- **Cover picker automation has a precise evidence limit.** The browser image chooser hung; the installed native picker opened but computer-use accessibility could not inspect it, so it was canceled and the app restarted without saving an image. Actual UI selection/save/restart remains unverified. Preprocessing and native save/reopen/invalid-payload tests pass; no mock or store test should be relabeled as picker acceptance. See B109.
- **The shell owns practice-panel tucking across workspaces.** Browsing Today/Pieces/Studio/Settings automatically minimizes an expanded Rep Counter into the bottom practice-tools pill. Returning to Score/Warmups restores it only if the shell tucked it; a manual closed/minimized choice stays respected, and the user can manually restore it while browsing. The earlier Studio-local unmount cleanup could reopen the Rep Counter over the library, so it is removed in favor of one shell owner. `DockProvider` records ephemeral user-action provenance and `RepPanel` identifies automatic reveal separately. Manual restore/minimize/close remain respected, including persisted and delayed-load cases. Independent final dock/library review passes with no remaining source blockers; 222 broad dock/Shell tests pass, and the two additional policy cases are included in the passing 2,595-test full frontend suite.
- **v11 installation preserves the exact live database.** Post-install `post-install-audit.json` confirms schema 21, integrity OK/FK0, exact equality of all 46 tables and byte-identical DB SHA-256 `b285c5a0624769aeccc27a2543f9b414eb1b22750fa2c551e60f02cfee553cb4`. Counts remain **11 pieces / 352 blocks / 3,237 reps / 63 sessions / 12,233 events / zero open sessions or blocks**. Both fresh launches and canceled profile/purchase/picker checks made no practice or preference changes. Backup `/Users/c3/Library/Application Support/Praelude Release Backups/v11.0.0-2026-09-07-2340/pre-v11.0.0.db` (35,004,416 bytes), SHA-256 `b285c5a0624769aeccc27a2543f9b414eb1b22750fa2c551e60f02cfee553cb4`; rollback `/Users/c3/Library/Application Support/Praelude Release Backups/v11.0.0-2026-09-07-2340/Praelude-10.1.0-rollback.zip` (9,778,474 bytes), SHA-256 `1b9647ce671feab84e23c0da66d0fd5351e19c1f2fc8e2908432e252a7e24d40`. Pre-install audit was captured 2026-09-07 at 23:41 EDT with the native app quit and zero open rows.
- **v11 release gates and source identity.** Frontend **2,595 passed / 1 skipped** across **212 passed files / 1 skipped**; native **1,106 passed / 18 ignored plus integrations** with four test workers; TypeScript, production/native builds, format and strict all-target/all-features Clippy pass. Final independent dock/library review found no remaining source blockers; broad dock/Shell checks passed 222, with the two additional policy cases included in the passing full suite. DMG `releases/v11.0.0/Praelude-11.0.0.dmg`: **10,691,711 bytes**, SHA-256 `dc51618396ecf532efa0a058a86f77a7451e186cac610c9d745f28da2d610417`. Runtime implementation `a33858139430b99bd22c8a2d0fa06fa6b1b2f4f1`; the app was built from the same unchanged production files immediately before that commit. Installed arm64 CDHash `eb19805326b4008e3a5cac1ed6bfcf472c870e98`; ad-hoc signed, not notarized; privacy strings verified. Release tag `v11.0.0` points to `c8e31b8e14bd3470d99be4852f7687e7a5a53f63`; the tag and private `origin/main` were pushed successfully. Read Git for the moving main tip after documentation-only follow-ups.
- **Living docs are a current entry point, not stacked installed-status history.** v11 condenses the portable summary, tutorial, Command Center and repo AGENTS. Historical release facts stay in Changelog/version records; old roadmap handoff text is explicitly archived. The tutorial now matches installed v11.0.0; native Score/Variants and cover-picker acceptance remain explicit limits.
...
- **Native v10.0.5 Score became available after the renewed permission gate (2026-09-07).** A fresh native Today→Score navigation loaded the real Scherzo No.2 score, and selecting Rolled Chords displayed the complete v10.0.5 bar without overlap at the existing desktop size. The previous waiting-on-Desktop-permission boundary is cleared for that launch. This is real WKWebView/passage rendering evidence, not microphone/Steinway acceptance and not proof of the subsequent v10.1 library UI.

- **v10.0.5 is installed with exact data preservation; native Score awaits renewed macOS permission (2026-09-07).** All release gates pass and `/Applications/Praelude.app` freshly launches v10.0.5. The post-launch DB file and complete logical graph match the pre-install backup boundary (11 pieces / 352 blocks / 3,237 reps / 63 sessions / 12,233 events, zero open; schema21/integrity OK/FK0). TCC explicitly requested Desktop-folder access on launch; the native score worker waits in `read_dir` until that prompt is answered. Computer-use tooling refuses the OS permission app. Treat browser bar QA as passed and native real-score QA as pending until Christian allows the prompt; do not infer native acceptance from installation. Exact artifacts/audits are in `docs/qa/v10.0.5/` and the vault Changelog.

- **After the repository rename, cached Tauri permission paths can still reference the old checkout (2026-09-07).** The first v10.0.5 native gate refused to build because cached `tauri` output referenced `/Users/c3/codakiller/src-tauri/target/debug/build/.../app_hide.toml`. Clearing only the generated `tauri` package build artifacts with `cargo clean -p tauri` and `cargo clean --release -p tauri` allows regeneration at the canonical `/Users/c3/praelude` path. This is a build-cache issue, not a reason to restore the old project path or change compatibility identifiers.

- **v10.0.5 brings the essential set draft controls into the Score bar (2026-09-07).** Christian’s sketch defines the visible order: a bounded passage identity and close action, Start set, Goal (the existing practice focus), Metronome checkbox with start/target tempo, Variants and Settings. Passage tools stays within the small identity area. Completion basis/count and tuning remain in Settings; Goal/tempo/checkbox use the existing submit state, not a second draft. Layout tracks the actual Score workspace width with ResizeObserver at 1000/740/470px rather than relying on the full window width. CSS containment is deliberately avoided because supported older WKWebView versions can make it the containing block for fixed dialogs. Active-set notices follow the controls in DOM order so grid placement leaves them on their own final row. Source verification is recorded in `docs/qa/v10.0.5/README.md`; packaging/install status is recorded there and in the vault Changelog.
...
- **The next source build gives earned practice feedback a deliberate sound/animation hierarchy (2026-09-03).** Each durably committed interactive verdict now gets a short local Web Audio acknowledgement: a bright two-note spark for Clean and quiet neutral confirmations for Sloppy/Again. The audio context is explicitly unlocked in the synchronously invoked click/hotkey verdict path and scheduled only after the write commits, so WKWebView does not have to permit a late autoplay request. Spoken verdicts retain the established immediate native acknowledgement chime. An earned ordinary completion becomes a compact rising fanfare; a variant-chain stage earns a three-note arc; mastery gets the fullest four-note landing. Pausing or closing an unfinished set keeps only its quiet visual exit and never plays a reward sound. The generated palette is offline and best-effort—blocked or unavailable audio is swallowed and can never delay, fail or falsely confirm a rep write. Rep buttons also perform a short Clean spark/bloom, while set/chain overlays add deterministic geometric bursts. Reduced-motion users retain the prior single static fade. Focused frontend tests, TypeScript and production build are required before packaging; source is awaiting its install boundary.
...
- **v9.1.2 makes the passage window a working surface, not a narrow card
  ...
  controls remain compact. Full release gates and post-install DB audit pass; it is installed.
