...
function attemptSourceLabel(source: string | undefined): string {
  switch (source) {
    case "user_click": return "button";
    case "voice_hot_loop": return "voice";
    case "voice_draft": return "voice draft";
    case "brain_draft": return "Brain draft";
    case "import_review": return "reviewed import";
    case "migration_legacy": return "legacy import";
    case "system_schedule": return "system";
    default: return source ? plainLabel(source).toLocaleLowerCase() : "unknown source";
  }
}

function contractSourceLabel(source: string): string {
  switch (source) {
    case "migration_legacy": return "legacy migration";
    case "user_click": return "set setup";
    case "system_schedule": return "scheduled contract";
    default: return plainLabel(source);
  }
}
...
function attemptLineage(rep: Rep): string {
  ...
  if (changes > 0) return `Corrected · ${changes} active ${changes === 1 ? "change" : "changes"}`;
...
export function BlockRow({ block, onChanged, regions = [] }: BlockRowProps) {
  ...
  const masteryVerified = block.mastery_verified === true
    && block.mastery_status !== "unverified_legacy";
  const masterySatisfied = masteryVerified && block.mastery_status === "satisfied";
  const masteryTarget = block.effective_required_clean_streak
    ?? block.required_clean_streak;
  const range = block.m_start === block.m_end
  ...
  async function loadReps() {
    ...
    try {
      ...
      if (mounted.current && attemptRequest.current === generation) {
        setError(commandErrorMessage(reason, "The attempts could not be loaded."));
      }
    }
  }
  ...
  async function mutateRep(
    ...
    try {
      ...
      const refreshError = "The attempt saved, but practice history could not be refreshed.";
    ...
    <article className="history-block" aria-label={`Practice set for ${range}`}>
      <div className="history-block-summary">
        ...
        <span className="history-range">
          {block.m_start === block.m_end ? `m. ${block.m_start}` : `mm. ${block.m_start}–${block.m_end}`}
        </span>
        <span className="history-block-label">{block.label || "Unlabeled set"}</span>
        {(block.focus === "tempo" || block.use_metronome) && (
          <span className="history-tempo">{tempoPath ?? "Tempo not captured"}</span>
        )}
        <span className="history-count">{attempts} attempt{attempts === 1 ? "" : "s"}</span>
        <span className={`history-mastery ${masterySatisfied ? "is-mastered" : masteryVerified ? "is-pending" : "is-unverified"}`}>
          {masterySatisfied ? "Mastery verified" : masteryVerified ? "Mastery not yet" : "Legacy · mastery unverified"}
        </span>
      ...
      <div className="history-block-controls" aria-label="Captured set contract">
        <span><strong>Focus</strong> {plainLabel(block.focus)}</span>
        <span>{block.use_metronome ? "Metronome on" : "Metronome off"}</span>
        <span><strong>Captured contract</strong> {contractText}</span>
        <span>{reviewText}</span>
        <span><strong>Contract source</strong> {sourceText}</span>
        {region && <span><strong>Section</strong> {region.name}</span>}
      </div>

      {expanded && (
        <ul className="history-reps">
          {reps.length === 0 ? <li className="history-empty">No attempts logged.</li> : reps.map((rep, index) => {
            const ordinal = index + 1;
            const isBusy = busyRep === rep.id;
            const showTempo = (block.focus === "tempo" || block.use_metronome) && rep.bpm != null;
            return (
              <li key={rep.id} className={`history-rep ${rep.voided ? "is-voided" : ""}`}>
                <span className="history-rep-index">{ordinal}</span>
                {rep.voided ? (
                  <span className={`history-rep-verdict is-${rep.verdict}`}>{VERDICT_LABEL[rep.verdict]}</span>
                ) : (
                  <select
                ...
                <span className="history-rep-bpm">{showTempo ? `♩ ${rep.bpm}` : plainLabel(block.focus)}</span>
                <span className="history-rep-source">Logged by {attemptSourceLabel(rep.source)}</span>
                <span className="history-rep-lineage">{attemptLineage(rep)}</span>
                {rep.voided ? (
                  <span className="history-rep-note">{rep.note || "No note"}</span>
                ) : (
                  <EditableField
                    value={rep.note ?? ""}
                    placeholder="Add note"
                    ariaLabel={`Attempt ${ordinal} note`}
                    onSave={(note) => mutateRep(
                      rep.id,
                ...
                {!rep.voided && (
                  ...
                  >
                    <button type="button" className="history-delete" disabled={isBusy} aria-label={`Void attempt ${ordinal}`}>×</button>
                  </ConfirmDelete>
                )}
              </li>
            );
